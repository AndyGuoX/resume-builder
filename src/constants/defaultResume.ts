      ...
      entries: [
        {
          ...
          bullets: [
            createBulletNode(
              '主导构建工具从 Webpack 迁移至 RsPack，重构构建链路与缓存策略，打包耗时从 120s 降至 20s（↓83%），显著提升 CI/CD 效率与开发体验',
            ),
            createBulletNode(
              '搭建完整RAG流程（文本切分 → {Embedding} → 向量检索 → LLM生成），设计混合检索策略（关键词 + 向量召回），提升检索准确率与召回率',
            ),
            createBulletNode(
              '引入多Query改写与路由机制，增强复杂问题理解能力，构建分层模型架构：轻量模型负责分类/路由/摘要，主模型负责生成，兼顾性能与成本',
            ),
            createBulletNode(
              '基于 {Redis} + {MySQL} 实现对话记忆与上下文管理，数据分析能力智能化升级：支持自然语言直接查询埋点数据，业务侧数据获取效率提升 3倍+',
            ),
          ],
        },
        {
          ...
          bullets: [
            ...
            createBulletNode(
              '可视化与前端架构沉淀：封装通用 Hooks 与业务组件体系，构建可复用的数据可视化方案（基于 ECharts），提升图表表达能力与交互体验',
            ),
          ],
        },
      ],
      ...
      entries: [
        {
          ...
          subheading:
            '基于Qiankun搭建的微前端基座系统，负责职位管理、集团账号、数据中心等10+子应用，实现技术栈无关的独立开发与部署。',
          period: '2024.07 - 至今',
          tags: ['React', 'QianKun', 'Redux', 'Webpack', 'Axios', 'Micro-frontend'],
          bullets: [
            createBulletNode(
              '设计统一请求层（Axios封装+拦截器+错误码体系），实现接口异常治理，线上错误率降低 35%',
            ),
            createBulletNode(
              '设计跨应用通信机制（PostMessage+全局状态），解决复杂业务场景数据同步问题，减少重复请求 50%+',
            ),
            createBulletNode(
              '从0-1设计集团账号资产管理项目(复杂数据管理、逻辑处理)，支撑集团会员大客复杂业务场景',
            ),
